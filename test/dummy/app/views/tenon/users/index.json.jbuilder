json.records do
  json.array!(@users) do |user|
    unless user.super_admin? && !current_user.super_admin?
      ...
      unless user == current_user
        json.approval_link action_link(user.approved? ? "Unapprove" : "Approve", approve_user_path(user), icon: user.approved? ? "thumbdown" : "thumbup", class: "approve", confirm: user.approved? ? "Are you sure you want to unapprove this user?" : "Are you sure? The user will be notified by email.") if policy(user).edit?
      end
