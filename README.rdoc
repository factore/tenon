...
a scope (eg. an <tt>ActiveRecord::Relation</tt>) and a set of params.  They can
then apply their own internal logic to filter the passed scope.  For example,
consider the following call to an imagined <tt>PostFilterer</tt>:
...
After creating the new filter, it can be inserted into the controller
...
Here is an example of what an imagined +PostFilterer+ that extends
